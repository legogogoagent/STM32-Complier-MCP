...
class ESP32BridgeClient:
    ...
    def uart_start(self, baudrate: int = 115200) -> bool:
        """
        启动UART透传
        ...
        Returns:
            成功返回True
        """
        response = self._send_command(f"uart_start {baudrate}")
        if response.startswith("OK:"):
            return True
        raise BridgeError(f"UART start failed: {response}")

    def uart_stop(self) -> bool:
        """
        停止UART透传
        ...
        Returns:
            成功返回True
        """
        response = self._send_command("uart_stop")
        if response.startswith("OK:"):
            return True
        raise BridgeError(f"UART stop failed: {response}")

    def uart_send(self, data: bytes) -> int:
        """
        发送数据到UART
        ...
        """
        hex_data = data.hex()
        response = self._send_command(f"uart_send {hex_data}")
        if response.startswith("OK:"):
            match = re.search(r"Sent (\d+) bytes", response)
            if match:
                return int(match.group(1))
        raise BridgeError(f"UART send failed: {response}")

    def uart_recv(self, timeout: float = 1.0) -> bytes:
        """
        从UART接收数据
...
class ESP32RemoteFlasher:
    ...
    def flash_binary(
        self, binary_data: bytes, address: int = 0x08000000, verify: bool = True
    ) -> bool:
        """
        烧录二进制数据
